...
namespace MSHC.Math.Geometry
{
	public class Vec2d : IEquatable<Vec2d>
	{
		...
		{
			SetLength(System.Math.Min(max, GetLength()));
		}

		public void DoMinLength(double min)
		{
			SetLength(System.Math.Max(min, GetLength()));
		}
		...
		{
			double cosTheta = System.Math.Cos(rads);
			double sinTheta = System.Math.Sin(rads);
